...
# wandb logging
wandb_log = False    # 关闭wandb（新手无需）
wandb_project = 'shakespeare-char'
wandb_run_name = 'mini-gpt'
# data
dataset = 'shakespeare_char'  # 关键修改：使用莎士比亚数据集
gradient_accumulation_steps = 1  # 关闭梯度累积（新手无需）
...
backend = 'nccl'
# system
device = 'cuda' if torch.cuda.is_available() else 'cpu'  # 自动检测设备（无GPU自动用CPU）
dtype = 'float16' if torch.cuda.is_available() else 'float32'  # 混合精度（CPU用float32）
compile = False    # 关键修改：关闭模型编译，避免兼容问题
# -----------------------------------------------------------------------------
